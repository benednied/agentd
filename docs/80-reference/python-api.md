...
| `register_node(node)` / `list_nodes()` | Register and inspect logical worker nodes. |
...
| `request_refinement(run_id, question)` | In-memory bounded refinement request record. |
| `report_blocker(run_id, blocker, retryable=True)` | In-memory bounded blocker record. |
...
for `Job`, `ExecutionContract`, `WorkerNode`, `ResourceVector`, `QuotaPool`,
`QuotaBudget`, `QuotaReservation`, `WorkspaceLease`, `Checkpoint`,
`ResumeCapsule`, `RunRecord`, `RunObservation`, and `RunResult`. It also lists
