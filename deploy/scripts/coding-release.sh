...
set -euo pipefail
if [[ $# != 2 || ( "$1" != activate && "$1" != rollback ) ]]; then
...
if [[ -L "$current" ]]; then
  ...
  if systemctl --user is-active --quiet agentd-coding-controller.service; then
    ...
    python3 -c 'import json,sys; d=json.load(sys.stdin); assert d["controller_live"] and d["draining"] and not d["unresolved_runs"] and d["workers"] and all(w["fresh"] and w["active_runs"] == 0 for w in d["workers"]), "still draining or ownership unresolved; let reconciliation finish before retry"' <<< "$report"
  fi
fi
